...
- **Classic** — countdown 120s (Time's up), timer luôn chạy kể cả khi pause/alt-tab
- **Zen** — count-up, không giới hạn thời gian, timer dừng khi pause
- **Gravity** — ô rơi theo trọng lực, count-up, timer dừng khi pause
...
- **Bug #2:** Timer Zen/Gravity/Mutation dừng khi pause (dùng `accumulated_time += delta`); Virus timer cũng dừng khi pause
...
1. ✅ **Kích cỡ ô trong màn chơi quá bé** → đã căn giữa + padding 20px
2. ✅ **Timer trong mode không có Time's up vẫn chạy khi pause** → đã fix bằng `accumulated_time`
3. ✅ **Mutation mode — ô Joker vẫn hiện điểm khi kéo box select** → đã fix via `get_effective_value()`
4. ✅ **Mutation mode — refill chưa sinh ô đặc biệt** → đã fix via `TileFactory`
5. ✅ **Pause menu — nút Leave** → về main menu (nút Quit trên GameOver cũng về main menu)
